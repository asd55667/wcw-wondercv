...
export default class _Axios {
  ...
  setInterceptors() {
    ...
    // (Object.keys(this.handler) as (keyof Handler)[]).forEach((method) => {
    //   this.handler[method].forEach((pair) => {
    //     this.axios.interceptors[method].use(pair.fulfilled, pair.rejected);
    //   });
    // });
  }
  ...
  addInterceptor(interceptor: Interceptor) {
    if (_Axios.isRequest(interceptor)) this.handler.request.push(interceptor);
  ...
  // get<I extends string>(inp: I): Pick<AxiosIOMethods, 'get'> {
  //   this.axios.get(inp);
  // }
}

// const axios1: AxiosInstance = axios.create();
// _Axios.prototype.get = axios1.get;
// export default function Axios(
//   options: AxiosRequestConfig,
//   interceptors: Interceptor[]
// ): _Axios & AxiosIOMethods {
//   return new _Axios(options, interceptors) as _Axios & AxiosIOMethods;
// }
// (METHODS as (keyof AxiosIOMethods)[]).forEach((method) => {
//   _Axios.prototype[method] = axios.create({})[method];
// });
