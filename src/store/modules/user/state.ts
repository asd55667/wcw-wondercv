// import { ref } from 'vue';
import type { CvModule, BasicModule, SingularModule, PluralModule, UserInfo } from '#/store';

const cvModules: CvModule[] = [
  // {
  //   id: 1,
  //   tag: 'icon',
  //   name: '校徽',
...
const jobModule: PluralModule[] = [
  {
    ...
    desc: '<p>watchdog文件系统监控</p><ul><li>内容安全，监控后台的图片上传目录，过滤违规图片</li></ul><p>背单词小程序</p><p>基本功能：辅助中学生背颂中考单词、周期复习</p><p>技术栈： 原生小程序、westore、flask、 nginx</p>',
  ...
  },
  {
    ...
    location: { desc: '所在城市', value: '上海' },
    desc: '<p>真实/仿真数据采集，处理</p><ul><li>跟车采集真实卡车数据</li><li>进程通信导出仿真数据</li></ul><p>检测/分割动态库接口</p><ul><li>单目测距， yolo批量预测</li><li>用libtorch将bisenet模型从py导出至cpp</li></ul>',
    update: new Date('2020-08-25'),
  },
];

const projectModule: PluralModule[] = [
  {
    ref: true,
  ...
  },
  {
    ...
    location: { desc: '所在城市', value: '上海' },
    desc: '<p>socket通信，http协议，哈希摘要，布隆过滤器，openssl, pthread互斥量；</p>',
    update: new Date('2020-08-25'),
  },
];

const eduModule: PluralModule[] = [
  {
    ref: true,
...
const user: UserInfo = { name: '', avatar_url: '' };
